"""
Training script for PPO agent - MAC FRIENDLY VERSION
"""
...
def train_ppo(num_episodes: int = 500, save_freq: int = 100, save_dir: str = 'results/ppo'):
    os.makedirs(save_dir, exist_ok=True)
    env = StudentSimulator(max_problems=20)
    agent = PPOAgent(state_size=6, action_size=6, learning_rate=0.0003, gamma=0.99, gae_lambda=0.95, clip_epsilon=0.2, entropy_coef=0.01, value_coef=0.5, ppo_epochs=10, mini_batch_size=64)
    episode_rewards = []
    episode_lengths = []
    skill_gains = []
    accuracies = []
    engagement_scores = []
    print("🚀 Starting PPO Training (MAC FRIENDLY)...")
    ...
    for episode in tqdm(range(num_episodes), desc="Training"):
        ...
        while not done:
            action, log_prob, entropy, value = agent.act(state, training=True)
        ...
        if (episode + 1) % 100 == 0:
            ...
            agent.save(f"{save_dir}/ppo_checkpoint_{episode+1}.pth")
