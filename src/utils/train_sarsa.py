...
def train_sarsa(num_episodes=500, save_dir='results/sarsa'):
    os.makedirs(save_dir, exist_ok=True)
    env = StudentSimulator(max_problems=20)
    agent = SARSAAgent(state_size=6, action_size=6, learning_rate=0.001, gamma=0.99, epsilon_start=1.0, epsilon_end=0.01, epsilon_decay=0.995)
    
    episode_rewards, episode_lengths, skill_gains, accuracies, engagement_scores = [], [], [], [], []
    
    print("🚀 Starting SARSA Training (On-Policy TD Learning)...")
    ...
    for episode in tqdm(range(num_episodes), desc="Training"):
        ...
        while not done:
            ...
            if not done:
                next_action = agent.act(next_state, training=True)
            else:
                next_action = 0
        ...
        if (episode + 1) % 100 == 0:
            ...
            agent.save(f"{save_dir}/sarsa_checkpoint_{episode+1}.pth")
    
    agent.save(f"{save_dir}/sarsa_final.pth")
...
def plot_training_results(metrics, save_dir):
    ...
    fig.suptitle('SARSA Training Results', fontsize=16)
