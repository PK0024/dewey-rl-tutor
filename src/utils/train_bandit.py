...
from agents.contextual_bandit_agent import ContextualBanditAgent
from tqdm import tqdm
import matplotlib.pyplot as plt
import json

def train_bandit(num_episodes=500, save_dir='results/bandit'):
    os.makedirs(save_dir, exist_ok=True)
    env = StudentSimulator(max_problems=20)
    agent = ContextualBanditAgent(state_size=6, action_size=6, exploration_param=2.0)
    
    episode_rewards, episode_lengths, skill_gains, accuracies, engagement_scores = [], [], [], [], []
    
    print("🚀 Starting Contextual Bandit Training (UCB Exploration)...")
    ...
    for episode in tqdm(range(num_episodes), desc="Training"):
        ...
        if (episode + 1) % 100 == 0:
            ...
            agent.save(f"{save_dir}/bandit_checkpoint_{episode+1}.json")
    
    agent.save(f"{save_dir}/bandit_final.json")
...
def plot_training_results(metrics, save_dir):
    ...
    fig.suptitle('Contextual Bandit (UCB) Training Results', fontsize=16)
