"""
DQN with Aggressive Learning (High LR, No Exploration)
"""
import sys, os
...
def train_dqn_aggressive(num_episodes=500, save_dir='results/dqn_aggressive'):
    os.makedirs(save_dir, exist_ok=True)
    env = StudentSimulator(max_problems=20)
    agent = DQNAgent(state_size=6, action_size=6, learning_rate=0.005, gamma=0.95, epsilon_start=0.1, epsilon_end=0.01, epsilon_decay=0.99, memory_size=5000, batch_size=32, target_update_freq=5)
    
    episode_rewards, episode_lengths, skill_gains, accuracies, engagement_scores = [], [], [], [], []
    
    print("🚀 Starting Aggressive DQN Training...")
    ...
    for episode in tqdm(range(num_episodes), desc="Training"):
        ...
        if (episode + 1) % 100 == 0:
            ...
            agent.save(f"{save_dir}/dqn_aggressive_checkpoint_{episode+1}.pth")
    
    agent.save(f"{save_dir}/dqn_aggressive_final.pth")
...
def plot_training_results(metrics, save_dir):
    ...
    fig.suptitle('Aggressive DQN Training Results', fontsize=16)
...
if __name__ == "__main__":
    train_dqn_aggressive(num_episodes=500)
